    ...
    internal static chd_error avHuff(byte[] source, byte[] dest)
{
    ...
    {
        // decode the audio
        chd_error err = DecodeAudio(audioChannels, audioSamplesPerBlock, source, srcOffset, audioHuffmanTreeSize, audioChannelCompressedSize, dest, audioChannelDestStart);
    ...
    if (treesize == 0xffff)
    {
        ...
        for (int channelNumber = 0; channelNumber < channels; channelNumber++)
        {
            ...
            if (curdest != null)
            {
                ...
                AudioBuffer audioBuffer = new AudioBuffer(settings, blockSize); //audio buffer to take decoded samples and read them to bytes.
                int read;
